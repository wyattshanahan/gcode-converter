def processGcode(fileName, scale, offsetX, offsetY):
    ...
    for i in range(s+1,lnCount):raw.append(content[i].split())
    for i in range(0,len(raw)): # iterate through the list
        currData = raw[i] # pull in the current list of data (G0, x, y)
        ...
        if x[0] == 'X':x = currData[1] #process data if an X value into the X list
        elif y[0] == 'X':x,y = currData[2],currData[3] # process data
        else: continue
        data += [[((float(x[1:]) * scale) + offsetX),((float(y[1:]) * scale) + offsetY),extrude]]
    if len(data) < 15: print(data)
    return data
def gCode(points):
    import turtle
    wn,t = turtle.Screen(),turtle.Turtle() # initialise the turtle and turtle graphics
    wn.tracer(0)
    for x, y, extrude in points: #iterate over the list, use processed data to draw with turtle
        if extrude:t.down()
        else:t.up()
        t.goto(x, y)
    wn.exitonclick()
